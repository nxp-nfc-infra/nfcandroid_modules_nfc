...
static bool ce_t4t_send_status(uint16_t status) {
  ...
  LOG(VERBOSE) << StringPrintf("%s: Status=0x%04X", __func__, status);
...
static bool ce_t4t_select_file(uint16_t file_id) {
  ...
  LOG(VERBOSE) << StringPrintf("%s: FileID=0x%04X", __func__, file_id);

  if (file_id == T4T_CC_FILE_ID) {
    LOG(VERBOSE) << StringPrintf("%s: Select CC file", __func__);
...
static bool ce_t4t_update_binary(uint16_t offset, uint8_t length,
  ...
  if ((starting_offset == 0) && (p_t4t->nlen > 0)) {
    ...
    if (ce_cb.p_cback) {
      ...
      LOG(VERBOSE) << StringPrintf("%s: Sent CE_T4T_NDEF_UPDATE_CPLT_EVT",
                                   __func__);
    }
...
static void ce_t4t_set_version_in_cc(uint8_t version) {
  ...
  LOG(VERBOSE) << StringPrintf("%s: version = 0x%02X", __func__, version);
  ...
  */
  ...
  if (ce_cb.mem.t4t.selected_aid_idx < CE_T4T_MAX_REG_AID) {
    ...
    if ((*p_cmd) == 0x00) {
      ...
      LOG(VERBOSE) << StringPrintf("%s: Not found matched AID", __func__);
      status_words = T4T_RSP_NOT_FOUND;
    }
    ...
    LOG(VERBOSE) << StringPrintf(
        "%s: Forward raw frame (SELECT APP) to wildcard AID handler", __func__);
  ...
  if (status_words) {
    ...
    if (status_words == T4T_RSP_CMD_CMPLTED) {
      ...
      LOG(VERBOSE) << StringPrintf("%s: T4T CE App selected", __func__);
    }
...
static void ce_t4t_data_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  if (ce_cb.mem.t4t.status & CE_T4T_STATUS_REG_AID_SELECTED) {
    LOG(VERBOSE) << StringPrintf("%s: Forward raw frame to registered AID",
                                 __func__);
    ...
    LOG(VERBOSE) << StringPrintf(
        "%s: Forward raw frame to wildcard AID handler", __func__);
    ...
    if (instruct == T4T_CMD_INS_SELECT) {
      ...
      if (select_type == T4T_CMD_P1_SELECT_BY_FILE_ID) {
        ...
        if (p_c_apdu->len < (T4T_CMD_MAX_HDR_SIZE + T4T_FILE_ID_SIZE)) {
          LOG(ERROR) << __func__ << ": Wrong length";
          GKI_freebuf(p_c_apdu);
          ce_t4t_send_status(T4T_RSP_WRONG_LENGTH);
          return;
        }
        ...
        LOG(ERROR) << StringPrintf("%s: Bad P1 byte (0x%02X)", __func__,
                                   select_type);
      ...
      if ((ce_cb.mem.t4t.status & CE_T4T_STATUS_CC_FILE_SELECTED) ||
        ...
        if ((uint32_t)length <= CE_T4T_MAX_LE) {
          ...
          if (length + offset > max_file_size) {
            if (offset < max_file_size) {
              ...
              LOG(VERBOSE) << StringPrintf(
                  "%s: length is reduced to %d by max_file_size (%d)", __func__,
                  length, max_file_size);
            } else {
              LOG(ERROR) << StringPrintf(
                  "%s: offset (%d) must be less than max_file_size (%d)",
          ...
          LOG(ERROR) << StringPrintf(
              "%s: length (%d) must be less than MLe (%zu)", __func__, length,
              CE_T4T_MAX_LE);
          length = 0;
        }
        ...
        LOG(ERROR) << StringPrintf("%s: File has not been selected", __func__);
      ...
      if (ce_cb.mem.t4t.status & CE_T4T_STATUS_NDEF_FILE_READ_ONLY) {
        LOG(ERROR) << StringPrintf("%s: No access right", __func__);
        ce_t4t_send_status(T4T_RSP_CMD_NOT_ALLOWED);
        ...
        if ((uint32_t)length <= CE_T4T_MAX_LC &&
          ...
          if (length + offset > ce_cb.mem.t4t.max_file_size) {
            LOG(ERROR) << StringPrintf(
                "%s: length (%d) + offset (%d) must be less than "
                "max_file_size (%d)",
          ...
          LOG(ERROR) << StringPrintf(
              "%s: length (%d) must be less than MLc (%zu)", __func__, length,
              CE_T4T_MAX_LC);
        ...
        LOG(ERROR) << StringPrintf("%s: NDEF File has not been selected",
                                   __func__);
        ce_t4t_send_status(T4T_RSP_CMD_NOT_ALLOWED);
      }
    } else {
      LOG(ERROR) << StringPrintf("%s: Unsupported Instruction byte (0x%02X)",
                                 __func__, instruct);
      ce_t4t_send_status(T4T_RSP_INSTR_NOT_SUPPORTED);
    }
  } else {
    LOG(ERROR) << StringPrintf("%s: Application has not been selected",
                               __func__);
    ce_t4t_send_status(T4T_RSP_CMD_NOT_ALLOWED);
  }
...
tNFC_STATUS CE_T4tSetLocalNDEFMsg(bool read_only, uint16_t ndef_msg_max,
  ...
  LOG(VERBOSE) << StringPrintf(
      "%s: read_only=%d, ndef_msg_max=%d, ndef_msg_len=%d", __func__, read_only,
      ndef_msg_max, ndef_msg_len);

  if (!p_ndef_msg) {
    p_t4t->p_ndef_msg = nullptr;

    LOG(VERBOSE) << StringPrintf("%s: T4T is disabled", __func__);
    return NFC_STATUS_OK;
  }
...
tCE_T4T_AID_HANDLE CE_T4tRegisterAID(uint8_t aid_len, uint8_t* p_aid,
  ...
  if (aid_len == 0) {
    LOG(VERBOSE) << StringPrintf("%s: callback for wildcard AID", __func__);
    ...
    LOG(VERBOSE) << StringPrintf(
        "%s: handle 0x%02x registered (for wildcard AID)", __func__,
        CE_T4T_WILDCARD_AID_HANDLE);
    p_t4t->p_wildcard_aid_cback = p_cback;
    return CE_T4T_WILDCARD_AID_HANDLE;
  }

  LOG(VERBOSE) << StringPrintf("%s: AID [%02X%02X%02X%02X...], %d bytes",
                               __func__, *p_aid, *(p_aid + 1), *(p_aid + 2),
                               *(p_aid + 3), aid_len);
  ...
  if (xx >= CE_T4T_MAX_REG_AID) {
    ...
    LOG(VERBOSE) << StringPrintf("%s: handle 0x%02x registered", __func__, xx);
  }
...
extern void CE_T4tDeregisterAID(tCE_T4T_AID_HANDLE aid_handle) {
  ...
  LOG(VERBOSE) << StringPrintf("%s: handle 0x%02x", __func__, aid_handle);
