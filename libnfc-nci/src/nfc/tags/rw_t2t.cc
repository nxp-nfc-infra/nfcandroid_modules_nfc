...
static void rw_t2t_proc_data(uint8_t conn_id, tNFC_DATA_CEVT* p_data) {
  ...
  if ((p_t2t->state == RW_T2T_STATE_IDLE) || (p_cmd_rsp_info == nullptr)) {
    LOG(VERBOSE) << StringPrintf("%s: RW T2T Raw Frame: Len [0x%X] Status [%s]",
                                 __func__, p_pkt->len,
  ...
  LOG(VERBOSE) << StringPrintf("%s: RW RECV [%s]=0x%x RSP", __func__,
  ...
  if (begin_state != p_t2t->state) {
    LOG(VERBOSE) << StringPrintf("%s: RW T2T state changed:<%s> -> <%s>",
                                 __func__,
...
#endif
      LOG(VERBOSE) << StringPrintf("%s: RW SENT [%s]=0x%x CMD", __func__,
                                   t2t_info_to_str(p_cmd_rsp_info),
                                   p_cmd_rsp_info->opcode);
...
static void rw_t2t_process_error(void) {
  ...
  LOG(VERBOSE) << StringPrintf("%s: State=%u", __func__, p_t2t->state);
  ...
  if ((!p_t2t->check_tag_halt) && (rw_cb.cur_retry < RW_MAX_RETRIES)) {
    ...
    LOG(VERBOSE) << StringPrintf("%s: T2T retransmission attempt %i of %i",
    ...
    if (p_t2t->check_tag_halt) {
      LOG(VERBOSE) << StringPrintf("%s: T2T Went to HALT State!", __func__);
    } else {
      LOG(VERBOSE) << StringPrintf(
...
tNFC_STATUS rw_t2t_sector_change(uint8_t sector) {
  ...
  if (status == NFC_STATUS_OK) {
    ...
    LOG(VERBOSE) << StringPrintf("%s: Sent Second Command", __func__);
    nfc_start_quick_timer(
        &p_t2t->t2_timer, NFC_TTYPE_RW_T2T_RESPONSE,
...
tNFC_STATUS rw_t2t_read(uint16_t block) {
  ...
  if (status == NFC_STATUS_OK) {
    p_t2t->block_read = block;
    LOG(VERBOSE) << StringPrintf("%s: Sent Command for Block=%u", __func__,
                                 block);
  }
...
tNFC_STATUS RW_T2tRead(uint16_t block) {
  ...
  if (status == NFC_STATUS_OK) {
    ...
    LOG(VERBOSE) << StringPrintf("%s: RW_T2tRead Sent Read command", __func__);
  }
...
tNFC_STATUS RW_T2tWrite(uint16_t block, uint8_t* p_write_data) {
  ...
  if (status == NFC_STATUS_OK) {
    ...
    if (block < T2T_FIRST_DATA_BLOCK)
      ...
      p_t2t->b_read_data = false;
    LOG(VERBOSE) << StringPrintf("%s: RW_T2tWrite Sent Write command",
                                 __func__);
  }
...
tNFC_STATUS RW_T2tSectorSelect(uint8_t sector) {
  ...
  if (status == NFC_STATUS_OK) {
    ...
    LOG(VERBOSE) << StringPrintf("%s: Sent Sector select first command",
                                 __func__);
  }
